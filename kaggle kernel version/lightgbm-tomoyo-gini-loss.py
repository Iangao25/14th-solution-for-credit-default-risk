...
def bureau_and_balance(num_rows = None, nan_as_category = True, threshold = 0.8):
    ...
    num_aggregations = {
        ...
        'AMT_CREDIT_DEBT_RATE': ['max','min','mean']
    }
    for cat in catoge_cols:
        num_aggregations[cat+'_TARGET_ENCODE'] = ['min', 'max']
...
def pos_cash(num_rows = None, nan_as_category = True, threshold=0.8):
    ...
    feature = feature[['POS' in feat for feat in feature['feature']]]
    feat_num = int(len(feature['feature']) * (1 - threshold))
    feat_drop = feature.tail(feat_num)['feature'].tolist()
    pos_agg = pos_agg.drop(feat_drop, axis=1)
    gc.collect()
    """
...
def installments_payments(num_rows = None, nan_as_category = True, threshold=0.8):
    ...
    """
    feature = pd.read_csv('../input/featureselectionhomecredit/REST.CSV')
    feature = feature[['INS' in feat for feat in feature['feature']]]
    feat_num = int(len(feature['feature']) * (1 - threshold))
    feat_drop = feature.tail(feat_num)['feature'].tolist()
    ins_agg = ins_agg.drop(feat_drop, axis=1)
    gc.collect()
    """
...
def credit_card_balance(num_rows = None, nan_as_category = True, threshold=0.8):
    ...
    """
    feature = pd.read_csv('../input/featureselectionhomecredit/REST.CSV')
    feature = feature[['CC' in feat for feat in feature['feature']]]
    feat_num = int(len(feature['feature']) * (1 - threshold))
    feat_drop = feature.tail(feat_num)['feature'].tolist()
    cc_agg = cc_agg.drop(feat_drop, axis=1)
    gc.collect()
    """
...
def kfold_lightgbm(num_folds, submission_file_name, stratified = False):
    
    train_df = pd.read_csv('../input/tomoyo-set/train/train.csv')
    test_df = pd.read_csv('../input/tomoyo-set/test/test.csv')
    feats = train_df.columns
    
    labels = pd.read_csv('../input/home-credit-default-risk/application_train.csv',usecols=['SK_ID_CURR','TARGET'])
    sub = pd.read_csv('../input/home-credit-default-risk/application_test.csv',usecols=['SK_ID_CURR'])
    ...
    fold_auc = []
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], labels['TARGET'])):
        train_x, train_y = train_df[feats].iloc[train_idx], labels['TARGET'].iloc[train_idx]
        valid_x, valid_y = train_df[feats].iloc[valid_idx], labels['TARGET'].iloc[valid_idx]
        ...
        clf.fit(train_x, train_y, eval_set=[(train_x, train_y), (valid_x, valid_y)],
            eval_metric=evalerror, verbose=1000, early_stopping_rounds=1000)
    ...
    print('Full AUC score %.6f' % roc_auc_score(labels['TARGET'], oof_preds))
    labels['TARGET'] = oof_preds
    labels[['SK_ID_CURR', 'TARGET']].to_csv('Tomoyo_GINI_TRAIN.csv', index=False)
    print('Average AUC score %.6f' % np.mean(fold_auc))
    print('standard deviation %.6f' % np.std(fold_auc))
    # Write submission file and plot feature importance
    sub['TARGET'] = sub_preds
    sub['SK_ID_CURR'] = sub['SK_ID_CURR'].astype('uint32').values
    sub[['SK_ID_CURR', 'TARGET']].to_csv("Tomoyo_GINI_TEST.csv", index=False)
...
def main(debug = False):
        ...
        feat_importance = kfold_lightgbm(num_folds=5,submission_file_name='Tomoyo_GINI_features_importance.csv',stratified=False)
